...
Composer resolves the lock file against PHP 8.2 so releases remain installable on
every supported PHP version. If `composer install` fails locally, check `php -v`
first.
